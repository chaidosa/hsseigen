    ...
    for(int i = n;i >= 1; i--){
        ...
        if(tch.size() == 0)
        {
            ...
            else
            {
                ...
                //T*T'
                cblas_dgemm(CblasRowMajor,CblasNoTrans,CblasTrans,A->uSizes[left-1].first,A->uSizes[left-1].first,A->bSizes[left-1].second,1,tempT,A->bSizes[left-1].second,tempT,A->uSizes[left-1].first,1,tempTTt,A->uSizes[left-1].first);                
           ...
           //here think tempVec as Sp stored in the stack
            ...
            for(int j=left-1;j >= desc[left];j--){
                ...
                if(j== temp_ch[1]){
                    ...
                    cblas_dgemm(CblasRowMajor,CblasNoTrans,CblasTrans,Sp_row,Sp_row,Sp_col,1,Sp,Sp_col,Sp,Sp_row,1,tempSST,Sp_row);
