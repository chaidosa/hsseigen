...
// step 2: deflate  close eigenvalue
    ...
    if(n1 < n){
       ...
       //sort d2 array and return the set of indices where the elements resided in the unsorted array
       //also rearrange v2 array elements based on the indices returned 
	//-------------------------------------
    ...
    int lamSize = Lam1.size() + Lam2.size() + org_size; //assuming org_size =Lam3 size
    ...
    if(n1 < n){    
        ...
        memcpy(T, Tempt.data(), sizeof(int)*Tempt.size());
        res->T = T;
